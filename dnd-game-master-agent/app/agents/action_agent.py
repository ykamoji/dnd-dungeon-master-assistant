...
action_executor = Agent(
    ...
    3. If you need module rules or scene context, call story_agent — but ask ONLY
       ...
       Tomb-of-Annihilation content and cannot look anything up by ID.
    4. Resolve the action under D&D 5e rules: roll attacks/saves, apply AC, modifiers,
    ...
    Issue the real tool call and wait for its response before resolving anything. If
    a needed tool returns nothing, say so in `narrative` instead of inventing data.
    ...
    Rules: always show your math; be specific about dice and modifiers; omit a
    character from `party` entirely rather than guessing their hp.

    CRITICAL: ALWAYS return the JSON object and nothing else — no prose before or
    after it. If information is missing or you would ask the player a question, put
...
class ActionChecker(BaseAgent):
    ...
    async def _run_async_impl(
        ...
        if error:
            feedback = "Rejected by action_checker: " + error
